...
public class FlowerShop {
   ...
   public boolean topNSame(int n, Flower[] otherInventory) {
    Flower[] flowerInventorySorted = sortByQuantity(flowerInventory);
    Flower[] otherInventorySorted = sortByQuantity(otherInventory);
    for (int i = 0; i < n; i++) {
      if (!flowerInventorySorted[i].getName().equals(otherInventorySorted[i].getName())) {
        return false;
      }
    }
    return true;
